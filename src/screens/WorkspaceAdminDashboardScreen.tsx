...
export default function WorkspaceAdminDashboardScreen({ onNavigate, onSelectProofreader, onSetFiles, onSetLrfFlowActive, onSetIfuFlowActive, adminModule, onSetAdminModule }: Props) {
  ...
  const filteredRows = moduleRows.filter(row => {
    ...
    const matchesWorkflow = workflowFilter === 'ALL' || row.workflow === workflowFilter
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden animate-fade-in" style={{ backgroundColor: C.bg }}>
      ...
      <AdminSidebar active="dashboard" userRole="workspace-admin" onNavigate={onNavigate} />
      <div className="flex-1 overflow-y-auto">
      <div className="px-8 py-8 flex flex-col gap-6 w-full" style={{ maxWidth: 1200, margin: '0 auto' }}>
        {/* Header */}
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-slate-500">Monitor {isIfuModule ? 'IFU document comparison' : 'label'} compliance runs for your workspace · <span className="font-semibold text-slate-600">DePuy CSV Team</span></p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-5">
          {flashCards.map(card => (
            ...
            >
              <div>
                <p className="text-xs font-semibold mb-1" style={{ color: 'rgba(255,255,255,0.7)' }}>{card.label}</p>
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="col-span-2 bg-white rounded-xl p-5 border border-slate-200 shadow-sm flex flex-col justify-between">
            <div className="flex justify-between items-center mb-3">
              <div>
                ...
                <p className="text-xs text-slate-400">Runs per day · DePuy CSV Team</p>
              </div>
            ...
            <div className="relative w-full flex items-center justify-center">
              <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="w-full h-auto">
                <defs>
                  <linearGradient id="chartGradientWA" x1="0" y1="0" x2="0" y2="1">
                ...
                <path d={areaD} fill="url(#chartGradientWA)" />
                ...
                {points.map((p, idx) => (
                  <g key={idx}>
                    ...
                    />
                    <text x={p.x} y={chartHeight - 4} textAnchor="middle" className="text-[10px] font-medium fill-slate-400">
                      {p.day}
                    </text>
                  </g>
                ))}
          ...
          {/* Workspace Health — no "create workspace" option */}
            ...
            <div>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                  ...
                  <span className="text-xs font-bold text-slate-800">DePuy CSV Team</span>
                </div>
                <div className="flex justify-between items-center border-b border-slate-100 pb-2">
          ...
          <div className="p-5 border-b border-slate-100 flex flex-col gap-4">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-slate-400">{isIfuModule ? 'IFU document comparisons' : 'Label comparisons'} completed by your team</p>
              </div>
              <div className="flex items-center gap-3">
                <button
                  ...
                  onClick={() => {
                    ...
                    a.download = 'proofx-workspace-activity.csv'
            ...
            <table className="w-full text-sm table-fixed" style={{ minWidth: 860 }}>
              ...
              <thead>
                <tr style={{ backgroundColor: C.grayBg, borderBottom: `1px solid ${C.border}` }}>
                  {['DATE / TIME', 'PROOFREADER', 'MASTER FILE', 'REVISED FILE', 'MODE', 'PAIRS', 'FINDINGS', ...(isIfuModule ? ['LANG COUNT'] : []), 'WORKFLOW', 'RUN STATUS', 'ACTIONS'].map(h => (
              ...
              <tbody>
                {filteredRows.length === 0 ? (
                  <tr>
                    <td colSpan={isIfuModule ? 11 : 10} className="px-3 py-8 text-center text-xs text-slate-400 italic">No matching records found.</td>
                  </tr>
                ) : (
                    ...
                    <div key={i} style={{ display: 'contents' }}>
                      ...
                      {expandedRows[i] && (
                        <tr className="bg-slate-50/50">
                          <td colSpan={10} className="px-8 py-4 border-t border-b border-slate-200">
